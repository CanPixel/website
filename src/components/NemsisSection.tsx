...
export default function NemsisSection() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        const q = query(projectsCollection, where('properties.band', '==', 'NEMSIS'));
  ...
  if (loading) return <div className="text-center">Loading NEMSIS projects...</div>;
  ...
  return (
    <div className="relative rounded-lg border-2 border-primary p-8 pt-12">
        ...
        <p className="text-center text-lg text-muted-foreground max-w-3xl mx-auto mb-6">
            Rock and electronic music fusion.
        </p>
        ...
        {projects.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground">No NEMSIS projects found.</p>
        )}
    </div>
  );
}
